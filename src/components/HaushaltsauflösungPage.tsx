...
import { fallbackHaushaltsaufloesung, services, Service } from '../data/services';
...
export default function HaushaltsauflösungPage() {
  ...
  // Initialize with fallback data for SSR - allows prerendering with real content
  const [service, setService] = useState<Service | null>(fallbackHaushaltsaufloesung || null);

  const rawSlug = location.pathname.replace('/', '');
  ...
  // During SSR with fallback data: render full page content
